...
from .services.llm_optional import rewrite_bullet_with_groq # Changed from rewrite_bullet_with_ollama
...
def generate(req: GenerateRequest):
  ...
  # Optional: if Groq configured, try rewriting a few bullets
  ...
  for section in ["experience", "projects"]:
    for item in tailored.get(section, []):
      ...
      for b in item.get("bullets", []):
        rb = rewrite_bullet_with_groq(b, keywords) # Changed from rewrite_bullet_with_ollama
